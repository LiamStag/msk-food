		...
		if ( ! empty( $formatted_meta ) ) {
			...
			foreach ( $formatted_meta as $meta ) {
				if ( $flat ) {
					...
					$meta_list[] = '
						<dt class="variation-' . sanitize_html_class( sanitize_text_field( $meta['key'] ) ) . '">' . wp_kses_post( $meta['label'] ) . 'good:</dt>
			...
			if ( ! empty( $meta_list ) ) {
				if ( $flat ) {
					...
					$output .= '<dl class="variation">' . implode( '', $meta_list ) . 'good</dl>';
				}
			}
		}
